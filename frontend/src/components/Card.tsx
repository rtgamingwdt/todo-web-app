...
export default function Card({ id, title, description, tag, due, className, _active }: { id: number, title: string, description: string, tag: string, due?: Date, className?: string, _active: boolean }) {

    const months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];

    const [active, setActive] = useState(_active);

    const { removeTodo } = useController();

    return (
        <div style={{ opacity: active ? "100%" : "25%" }} className={"duration-500 bg-card p-4 rounded-md border border-main relative" + (' ' + className ?? '')}>
            ...
            <div className="pt-4 flex gap-4">
                ...
                <button className="p-2 rounded-full bg-red-500" onClick={() => removeTodo(id)}><FaTrash /></button>
            </div>
        </div>
    )
}